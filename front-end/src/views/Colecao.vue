<template>
    <v-container>
        ...
        <v-row>
            <v-col v-for="collection in collections" :key="collection._id" cols="12" sm="6" md="4">
                <v-card class="ma-3 elevation-5">
                    <v-card-title class="headline">{{ collection.name }}</v-card-title>
                    <v-card-text>
                        <ul>
                            <li v-for="board in collection.boards" :key="board._id">{{ board.title }}</li>
                        </ul>
                    </v-card-text>
                    <v-card-actions>
...
export default {
    setup() {
        ...
        // Carrega as coleções existentes
        ...
        // Abre o diálogo de edição com os dados da coleção
        ...
        const deleteCollection = async (collectionId) => {
            try {
                ...
                loadBoards();  // Atualiza os quadros após a remoção de uma coleção
            } catch (error) {
        ...
        // Carrega as coleções e quadros ao montar o componente
        onMounted(() => {
            loadCollections();
            loadBoards();
        });
